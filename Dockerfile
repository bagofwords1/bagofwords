...
RUN apt-get update && \
    ...
    (ACCEPT_EULA=Y apt-get install -y --no-install-recommends msodbcsql18 || echo "WARN: msodbcsql18 install failed — ODBC Driver 18 will be unavailable") && \
    (ACCEPT_EULA=Y apt-get install -y --no-install-recommends msodbcsql17 || echo "WARN: msodbcsql17 install failed — ODBC Driver 17 will be unavailable") && \
