<template>
  <div class="flex h-screen justify-center py-20 px-5 sm:px-0" v-if="pageLoaded">
    <div class="w-full text-center sm:w-[400px]">
      ...
      <div class="px-10 py-6  border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm bg-white dark:bg-gray-900">
        <form @submit.prevent='submit' v-if="authMode !== 'sso_only'">
          <div class="field block mt-3">
            <input :placeholder="$t('auth.name')" id='name' v-model='name' class="border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 w-full h-10 text-sm focus:outline-none focus:border-blue-500"/>
          </div>
          <div class="field mt-3">
            <input :placeholder="$t('auth.email')" id='email' v-model='email' class="border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 w-full h-10 text-sm focus:outline-none focus:border-blue-500"/>
          </div>
          <div class="field mt-3">
            <input type='password' :placeholder="$t('auth.password')" id='password' v-model='password' class="border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 w-full h-10 text-sm focus:outline-none focus:border-blue-500"/>
          </div>
        ...
        <div class="mt-3" v-if="authMode !== 'local_only' && (googleSignIn || oidcProviders.length)">
          ...
          <div class="mt-3" v-if="googleSignIn">
            <button @click="signInWithGoogle" :disabled="loadingProvider !== null" class="w-full flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div class="mt-3 block text-xs border-t border-gray-100 dark:border-gray-800 pt-3">
        {{ $t('auth.termsPrefix') }}
