<template>
  <div class="min-h-screen bg-gray-50 py-10 px-4">
    <div class="w-full max-w-5xl mx-auto">
      <div class="mb-4">
        <h1 class="text-lg font-semibold">Select tables</h1>
        <p class="text-gray-500 text-sm">Choose which tables to enable</p>
      </div>
...
onMounted(async () => {
  if (!id.value) return router.replace('/integrations')
  loading.value = true
  try {
    const res = await useMyFetch(`/data_sources/${id.value}/schema`, { method: 'GET' })
...
async function handleSave() {
  ...
  saving.value = true
  try {
    const payload = tables.value.map(t => ({ ...t, datasource_id: id.value, pks: t.pks || [], fks: t.fks || [] }))
    const res = await useMyFetch(`/data_sources/${id.value}/update_schema`, { method: 'PUT', body: payload })
    if ((res.status as any)?.value === 'success') {
      router.replace(`/integrations/new/${id.value}/context`)
    }
  } finally {
    saving.value = false
  }
