<template>
  <div class="flex flex-col h-[calc(100vh-90px)] text-sm">
    ...
    <div class="flex-1 min-h-0 flex border border-gray-200 rounded-xl overflow-hidden bg-white">
      <!-- ── Pane 1: Tree (agents + instructions) ───────── -->
      <aside class="w-[320px] shrink-0 border-r border-gray-200 flex flex-col bg-gray-50/40">
        <!-- Search -->
        <div class="p-2.5 border-b border-gray-200/70 space-y-2">
          <div class="relative">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-0.5">
          <!-- Smart views -->
          <TreeGroup label="All instructions" icon="i-heroicons-square-3-stack-3d" :count="allInstructions.length"
                     :open="isOpen('all')" @toggle="expand('all')">
            <InstrLeaf v-for="ins in listFor('all')" :key="ins.id" :ins="ins" />
          </TreeGroup>

          <TreeGroup label="Skills" icon="i-heroicons-sparkles" :count="skillCount"
                     :open="isOpen('skills')" @toggle="expand('skills')">
          ...
          <TreeGroup label="Pending review" icon="i-heroicons-clock" :count="pendingCount" :count-accent="pendingCount > 0"
                     :open="isOpen('pending')" @toggle="expand('pending')">
          ...
          <TreeGroup label="Global / Org-wide" icon="i-heroicons-globe-alt" :count="globalCount"
                     :open="isOpen('global')" @toggle="expand('global')">
          ...
          <template v-for="agent in agents" :key="agent.id">
            <TreeGroup :label="agent.name" :count="agentCount(agent.id)" :pending="agentPending(agent.id)"
                       :open="isOpen('agent:' + agent.id)" @toggle="expand('agent:' + agent.id)">
              <template #icon>
                <span class="w-4 h-4 rounded flex items-center justify-center text-[10px] font-semibold shrink-0" :class="agentBadgeClass(agent.id)">{{ agentInitial(agent.name) }}</span>
              </template>
              <!-- tables (table-scoped rules) -->
              <TreeGroup
                v-for="t in (agentTables[agent.id] || [])" :key="t.key"
                :label="t.label" icon="i-heroicons-table-cells" :count="t.count" mono :indent="1"
                :open="isOpen('table:' + agent.id + ':' + t.key)" @toggle="expand('table:' + agent.id + ':' + t.key)"
              >
                <InstrLeaf v-for="ins in listForTable(agent.id, t.key)" :key="ins.id" :ins="ins" :indent="2" />
              </TreeGroup>
              <!-- agent-level (non table-scoped) -->
              <InstrLeaf v-for="ins in listForAgent(agent.id)" :key="ins.id" :ins="ins" :indent="1" />
              <div v-if="agentCount(agent.id) === 0" class="pl-8 py-1 text-[11px] text-gray-300 italic">No instructions yet.</div>
            </TreeGroup>
          </template>
        </div>

        <!-- Bulk bar -->
        <div v-if="selectedIds.size > 0" class="px-3 py-2 border-t border-gray-200/70 bg-white flex items-center gap-2 text-xs">
          ...
          <div class="flex-1 overflow-y-auto px-8 py-6 max-w-3xl">
            ...
            <div class="mt-6 pt-5 border-t border-gray-100">
              ...
              <div v-else class="flex flex-wrap gap-1.5">
                <span v-for="ds in detail.data_sources" :key="ds.id" class="inline-flex items-center gap-1 px-2 h-6 rounded-md bg-gray-100 text-gray-600 text-[11px]">
              ...
              <template v-if="(detail.references || []).length">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <span v-for="(ref, i) in detail.references" :key="i" class="inline-flex items-center gap-1 px-2 h-6 rounded-md bg-gray-100 text-gray-600 text-[11px] font-mono">
                    <UIcon :name="h.getRefIcon(ref.object_type)" class="w-3 h-3" /> {{ refLabel(ref) }}
                  </span>
                </div>
              </template>
            </div>
      ...
      <aside v-if="detail && showHistory" class="w-60 shrink-0 border-l border-gray-200 flex flex-col bg-gray-50/40">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-0.5">
          ...
          <div v-for="(v, i) in versions" :key="v.id" class="px-2.5 py-2 rounded-md hover:bg-white border border-transparent hover:border-gray-200 transition-colors">
...
const expand = (key: string, force?: boolean) => {
  ...
  if (key.startsWith('agent:')) buildAgentTables(key.slice('agent:'.length))
}
...
const buildAgentTables = (agentId: string) => {
  const map = new Map<string, { label: string; count: number }>()
  for (const ins of allInstructions.value) {
    if (!(ins.data_sources || []).some(d => d.id === agentId)) continue
...
// ── Leaf lists (search + status applied) ────────────────
...
const listForTable = (agentId: string, tableKey: string) => applyFilters(allInstructions.value.filter(i =>
  (i.data_sources || []).some(d => d.id === agentId) &&
  (i.references || []).some((r: any) => r.object_type === 'datasource_table' && String(r.object_id) === tableKey)
))
...
const agentBadgeClass = (id: string) => { const idx = agents.value.findIndex(a => a.id === id); return AGENT_COLORS[(idx >= 0 ? idx : 0) % AGENT_COLORS.length] }
  ...
  setup(props, { slots, emit }) {
    return () => createElement('div', {}, [
      createElement('button', {
        class: ['w-full flex items-center gap-1.5 h-7 rounded-md text-xs text-gray-600 hover:bg-gray-100/70 transition-colors min-w-0'],
  ...
  setup(props) {
    return () => {
      ...
      return createElement('button', {
        class: ['group w-full flex items-center gap-2 h-7 rounded-md text-xs transition-colors min-w-0',
          sel ? 'bg-blue-50/70 text-gray-900' : 'text-gray-600 hover:bg-gray-100/70'],
        ...
        createElement('span', {
          class: ['shrink-0 w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors',
            checked ? 'bg-gray-900 border-gray-900' : 'border-gray-300 opacity-0 group-hover:opacity-100'],
        ...
        createElement('span', { class: ['shrink-0 inline-flex items-center px-1.5 h-4 rounded text-[10px] font-medium', h.getLoadModeClass(ins.load_mode)] }, h.getLoadModeLabel(ins.load_mode)),
