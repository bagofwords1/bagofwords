<!--
  One query's detail view: header + workflow actions + parameter bar + the
  visual/data/code tabs.

  Extracted from the old standalone /queries/<id> page when the catalog moved
  into the /agents knowledge tree, so the tree's right pane and the deep-link
  route render the same thing instead of drifting into two detail views. The
  host supplies the id and listens for `changed` / `deleted` to keep its own
  list in step.
-->
<template>
  <div class="py-6">
...
// Every workflow action returns the updated entity; funnel them through one
// place so the host is told exactly once, with the row it should now show.
function applyDetail(v: any) {
  detail.value = v
  emit('changed', v)
}
...
function onModalSaved() {
  load().then(() => emit('changed', detail.value))
