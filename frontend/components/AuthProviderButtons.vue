<template>
  <div class="space-y-2" v-if="allProviders.length">
    <button
...
const props = withDefaults(defineProps<{
  ...
  // stop on a chooser. The caller resolves it, because on the flow that
  // matters it arrives nested inside `redirect` rather than beside it.
  loginHint?: string
}>(), {
  mode: 'sign-in',
  loginHint: '',
