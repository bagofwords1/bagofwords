<template>
  <section class="mt-4 rounded-md border border-gray-200 bg-gray-50/70 p-3 dark:border-gray-700 dark:bg-gray-800/40" data-testid="table-recent-prompts">
    <div class="flex items-start gap-2">
      <span class="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-300"><UIcon name="i-heroicons-chat-bubble-left-right" class="h-3 w-3" /></span>
      <div class="min-w-0">
        <h3 class="text-xs font-semibold text-gray-700 dark:text-gray-200">{{ t('tableErd.recentPrompts') }}</h3>
        <p class="mt-0.5 text-[10px] leading-relaxed text-gray-500">{{ t('tableErd.thisAgentOnly') }}</p>
      </div>
    </div>
    <ol class="mt-3 space-y-2">
      <li v-for="item in items" :key="item.execution_id" class="rounded border border-gray-200/80 bg-white p-2 dark:border-gray-700 dark:bg-gray-900/70">
    ...
    <p v-if="loading" class="py-2 text-[11px] text-gray-500" role="status">{{ t('tableErd.loadingPrompts') }}</p>
    <div v-else-if="error" role="alert" class="py-2 text-[11px] text-gray-500">{{ t('tableErd.promptsError') }} <button class="text-blue-600" @click="load">{{ t('tableErd.retry') }}</button></div>
    <p v-else-if="!items.length" class="py-2 text-[11px] text-gray-500">{{ t('tableErd.noPrompts') }}</p>
    <button v-if="!loading && !error && nextOffset !== null" class="pt-2 text-[11px] font-medium text-blue-600" @click="load">{{ t('tableErd.morePrompts') }}</button>
  </section>
</template>
