...
const props = withDefaults(defineProps<{
  modelValue?: boolean, suiteId: string, caseId?: string, agentId?: string,
  /** Show the title row's dismiss button. The right-hand pane has its own. */
  closable?: boolean,
...
// Mounted means open. A host that unmounts the editor closes it.
const isOpen = computed({ get: () => props.modelValue !== false, set: (v) => emit('update:modelValue', v) })
...
// Both signals: a modal host listens for update:modelValue, a pane for close.
...
// The host owns the footer — a modal renders Cancel/Save/Save-and-Run, the
// right-hand pane renders its own — so the actions and their pending state are
// exposed rather than duplicated in both.
defineExpose({ save, runNow, isSaving, isRunning, initialLoading, isEditing })
</script>
