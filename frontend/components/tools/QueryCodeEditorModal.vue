      ...
      <div class="flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <section class="flex-1 flex flex-col overflow-hidden min-h-0">
          <div v-if="canEditCode && activeTab === 'code'" class="h-full flex overflow-hidden">
           ...
           <QueryParamsPanel
             v-if="paramsSidebarOpen"
             class="w-[330px] flex-shrink-0 border-s"
             :specs="paramSpecs"
             :test-values="paramTestValues"
             :code="editorCode"
             :applied-params="appliedParams"
             :option-queries="optionQueries"
             :running="running"
             :run-mode="runMode"
             :error-msg="errorMsg"
             show-save
...
// Report queries a param may take its choices from (the filter-space
// pattern), with the columns of their current result.
const optionQueries = computed(() =>
  reportQueriesList.value
    .filter((q: any) => String(q.id) !== String(queryId.value || ''))
    .map((q: any) => ({
      id: String(q.id),
      title: q.title,
      columns: ((q?.default_step?.data?.columns || []) as any[])
        .map((c: any) => c.field || c.headerName).filter(Boolean),
    })))

const zeroRowsHint = computed(() =>
  zeroRowsHintFor(preview.value?.rows, appliedParams.value, paramSpecs.value, editorCode.value))

function cleanedParamSpecs(): any[] { return cleanParamSpecs(paramSpecs.value) }
