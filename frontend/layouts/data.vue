    ...
    <NuxtLayout name="default">
        <div class="flex justify-center ps-2 md:ps-4 text-sm">
            <div class="w-full max-w-7xl px-4 ps-0 py-4">
                ...
                <!-- Back link -->
                <NuxtLink to="/agents" class="inline-flex items-center gap-1 text-xs text-gray-400 dark:text-gray-600 hover:text-gray-600 transition-colors">
                        ...
                        <template v-else-if="!fetchError">
                            ...
                            <div v-if="integration?.description || useCan('update_data_source')" class="mt-2 flex items-center gap-2 group max-w-2xl">
                                <template v-if="editingDesc">
                                    <input
                                        ...
                                        v-model="descForm"
                                        type="text"
                                        class="flex-1 text-sm text-gray-600 dark:text-gray-400 border-b border-blue-400 bg-transparent outline-none py-0.5"
                                ...
                                <template v-else>
                                    <p
                                        ...
                                        :class="useCan('update_data_source') ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700' : ''"
                            ...
                            <div class="flex items-center gap-3 mt-4 flex-wrap">
                                <template v-if="(integration?.connections || []).length > 0">
                                    ...
                                    <span v-if="(integration.connections || []).length > 4" class="text-xs text-gray-400 dark:text-gray-600">
                                        +{{ integration.connections.length - 4 }}
                                    </span>
                                </template>
                                <span v-else class="text-xs text-gray-400 dark:text-gray-600 italic">No connections</span>

                                <template v-if="(catalog.shouldShow && catalog.count > 0) || connectionCount > 0">
                                    <template v-if="catalog.shouldShow">
                                        <span class="text-gray-300 dark:text-gray-600 select-none">·</span>
                                        <span class="text-xs text-gray-400 dark:text-gray-600">{{ catalog.label }}</span>
                                    </template>
                                    <span class="text-gray-300 dark:text-gray-600 select-none">·</span>
                                    <span class="text-xs text-gray-400 dark:text-gray-600">{{ connectionCount }} {{ connectionCount === 1 ? 'connection' : 'connections' }}</span>
                                </template>
                            </div>
                        </template>
                ...
                <div v-if="!isLoading && fetchError === 403" class="mt-8">
                    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-10 text-center">
