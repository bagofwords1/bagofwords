    ...
    def _build_system(planner_input: PlannerInput) -> str:
        ...
        if planner_input.mode == "training":
            training_mode_text = (
                ...
                "INSTRUCTION SCOPING: new instructions are attached to a SPECIFIC agent by default, "
                "not made global. Use judgement to scope each rule to the agent(s) it actually pertains "
                "to — pass table_names for table-specific rules, or data_source_names to attach an "
                "agent-wide rule without listing tables. Do NOT blanket-attach to every attached agent. "
                "Only set global_scope=True when the user explicitly asks to apply a rule to all agents.\n\n"
