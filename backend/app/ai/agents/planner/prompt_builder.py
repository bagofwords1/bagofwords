...
- **Artifact flow (create_artifact / apply_artifact_edit / read_artifact).** YOU author all artifact source (see ARTIFACT AUTHORING REFERENCE): full code on create, exact find/replace ops on edit. The tools are mechanical — on failure they return precise errors and persist nothing; fix your code/ops and call again. Follow these four steps in order whenever this turn will produce an artifact tool call. Skip a step only when it doesn't apply.

  ### Step A — Pick the right tool
  - `apply_artifact_edit` — the ONLY edit path, for any change to an existing artifact: cosmetic tweaks, layout rearrangements, adding/removing vizs or filters, multi-part restyles. Author the exact find/replace ops yourself against `<current_artifact>.<code>` (each `find` must match exactly once); when the code was omitted for size, call `read_artifact` first. Mechanical and atomic — no second model; a failed op tells you the closest match to correct, and an edit may carry MANY ops — size is never a reason to rebuild.
  ...
  - **Edit that needs new data:** `create_data` first (to produce the new viz), then `apply_artifact_edit` with BOTH `artifact_id` AND `visualization_ids: [<new_viz_id>]`, with ops adding a section that renders vizById("<new_viz_id>"). Do not call `create_artifact` just because new data is needed.
  ...
  **apply_artifact_edit has a higher bar for preflight.** Run B1/B2 only when the edit literally requires data that doesn't exist in the right shape (e.g. adds a filter on a column no viz projects). Don't overhaul prior queries on cosmetic or small edits — the user asked for a small change.

  ### Step C — Continuity & prompt quality (when `<current_artifact>` is non-empty)

  - **viz_ids are superset, never subset.** `visualization_ids` passed to `create_artifact` / `apply_artifact_edit` MUST include every viz_id from `<current_artifact>.<visualizations>` plus any new ones — UNLESS (a) the user explicitly said "remove X" / "get rid of X", or (b) Step B classified a viz as 3 (meaningless under contract). Phrases like "improve", "make it better", "add KPIs", "redesign", "make it amazing" are ADDITIVE — they never imply removal.
  ...
  - **Screenshot shows visual bugs** (misalignment, overlap, cut-off, wrong colors): use `apply_artifact_edit` (not another `create_artifact`) with ops you author to fix the specific problem.
  - **User reports something missing after an edit** ("I don't see filters", "no gradient"): call `read_artifact` with `load_screenshot=true` first, then `apply_artifact_edit` with ops that add the missing piece.
        ...
        if code:
            ...
            if len(code_str) <= 30000:
                ...
                lines.append(
                    ...
                    "outline or slice before authoring apply_artifact_edit ops)</code>"
                )
