    ...
    async def data_model_to_code(
        ...
        **Guidelines and Requirements**:
        ...
        2. **Data Source Usage**:
           ...
           - **Power BI connections**: `execute_query` needs the target semantic model — pass the schema table name (format `Dataset/Table`, exactly as shown in the schema) as the SECOND argument: `execute_query("EVALUATE Customers", "SalesModel/Customers")`. Alternatively pass `dataset_id=`/`workspace_id=` from the table's `<powerbi .../>` metadata. Never ask the user for these IDs. Push the filter AND the aggregation into the DAX itself (`SUMMARIZECOLUMNS` with a filter argument, or `CALCULATETABLE` - see the connection's DAX guide) so ONE query returns the answer. Never `EVALUATE` a table and then filter/`groupby` in pandas: executeQueries streams every row over REST, and that transfer is what makes Power BI queries slow.
