    ...
    async def data_model_to_code(
        ...
        **Guidelines and Requirements**:
        ...
        2. **Data Source Usage**:
           ...
           - **Connection-Table Mapping**: Each client_key corresponds to a specific database connection. The `<connection name="...">` tags in <ground_truth_schemas> show which tables belong to which connection. Match the connection name to the client_key suffix (e.g., `<connection name="postgresql-1">` → `ds_clients["...:postgresql-1"]`). Only query tables listed under that connection. When reusing code from a saved entity, a mention, or an earlier step, replace any ds_clients key it contains with the matching key from <connection_clients> — a key naming another agent does not exist in this run.
...
{reuse_directive}
    ...
    async def generate_inspection_code(
        ...
        **Constraints**:
        ...
        4. **Connection-Table Mapping**: Match `<connection name="...">` in schemas to the client_key suffix (e.g., `<connection name="postgresql-1">` → `ds_clients["...:postgresql-1"]`). Only query tables listed under that connection. When reusing code from a saved entity, a mention, or an earlier step, replace any ds_clients key it contains with the matching key from <connection_clients> — a key naming another agent does not exist in this run.
