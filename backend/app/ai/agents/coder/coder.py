    ...
    async def data_model_to_code(
        ...
        **Guidelines and Requirements**:
        ...
        2. **Data Source Usage**:
           ...
           - **Large or expensive scans (optional, out-of-core)**: `ds_clients["<client_key>"].execute_query_lazy("SOME QUERY")` returns a LazyFrame backed by disk instead of loading every row into memory. Push filters/aggregations down with `.sql("SELECT ... FROM data WHERE ...")` (refer to the result as `data`), then call `.to_df()` only on the final, reduced result. Prefer this when a query could return very large results; for normal-sized results use `execute_query`. The value returned by `generate_df` must still be a pandas DataFrame — always `.to_df()` the reduced LazyFrame before returning it or combining it with pandas.
