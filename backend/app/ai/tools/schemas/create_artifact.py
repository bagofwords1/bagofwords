...
class CreateArtifactInput(BaseModel):
    ...
    prompt: str = Field(..., description=(
        ...
        "PERSONALIZATION: personalization is a RUNTIME BINDING, never resolved text. Write the INSTRUCTION ('greet the viewing user by name via current_user, with a neutral "
        "fallback') — NEVER substitute the requester's actual name/email into this plan. The artifact is shared: every viewer gets their own identity at render time. "
        "If a preview/screenshot appears to lack personalization, that preview renders as an ANONYMOUS viewer — the fix is NEVER a literal name.\n\n"
