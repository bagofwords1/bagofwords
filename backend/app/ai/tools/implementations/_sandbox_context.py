...
• **Pre-built UI components** — all global, prefer these for speed but build custom components when the design requires it:
  ...
  - **`viz` prop (KPICard & SectionCard)** — pass the source visualization object and the card automatically shows a small "ⓘ" button that opens a clean popover with that viz's provenance (source, query/SQL, columns, row count, aggregation, active filters, id). ALWAYS pass `viz={visualizations[N]}` for the visualization a card is built from. For a card derived from multiple vizs, pass its primary one. No extra markup needed — the popover is built in.
