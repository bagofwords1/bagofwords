"""
LEGACY coder-based artifact editing — retired from the planner.

Kept for the MCP surface (external callers send English edit instructions and
cannot author diffs themselves) and for the shared diff helpers
(apply_search_replace_diff, _find_closest_match, build_pinned_decisions).
The planner-facing `edit_artifact` is now the MECHANICAL tool in
edit_artifact.py: planner-authored find/replace ops, no inner LLM.

Originally: edit_artifact tool - surgically edit artifact code using search/replace diffs.
...
from app.ai.tools.schemas.edit_artifact_legacy import (
    LegacyEditArtifactInput as EditArtifactInput,
    LegacyEditArtifactOutput as EditArtifactOutput,
)
...
class EditArtifactTool(Tool):
    ...
    def metadata(self) -> ToolMetadata:
        return ToolMetadata(
            ...
            # via edit_artifact (docs via edit_doc). The class remains for
