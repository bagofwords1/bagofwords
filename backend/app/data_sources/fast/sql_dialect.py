...
def explain_postgresql(conn, sql: str) -> Tuple[Optional[int], Optional[int], str]:
    """(rows, width_bytes, note) from `EXPLAIN (FORMAT JSON)`."""
    ...
    return int(plan.get("Plan Rows") or 0), int(plan.get("Plan Width") or 0), ""


def explain_mysql(conn, sql: str) -> Tuple[Optional[int], Optional[int], str]:
    """(rows, width_bytes, note) from classic `EXPLAIN`.
    ...
    for r in res:
        ...
        if v:
            rows = max(rows, int(v))
    return rows, ASSUMED_ROW_WIDTH_BYTES, "row width estimated (EXPLAIN provides none)"


def explain_mssql(conn, sql: str) -> Tuple[Optional[int], Optional[int], str]:
    ...
    if not plan_xml:
        return None, None, "SHOWPLAN_XML returned no plan"
    ...
    for el in root.iter():
        ...
        if rows is not None and width is not None:
            break
    if rows is None:
        return None, None, "no StatementEstRows in the plan"
    return rows, width, "" if width else "row width unavailable in the plan"


_STATEMENT_ID_SAFE = re.compile(r"[^A-Za-z0-9_]")


def explain_oracle(conn, sql: str) -> Tuple[Optional[int], Optional[int], str]:
    ...
    if row is None:
        return None, None, "no PLAN_TABLE row for the statement"
    ...
    if total_bytes is None or not rows:
        return rows, None, "plan reported no BYTES"
