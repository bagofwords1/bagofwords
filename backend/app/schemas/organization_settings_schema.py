...
class OrganizationSettingsConfig(BaseModel):
    ...
    enable_lazy_queries: FeatureConfig = FeatureConfig(value=False, name="Lazy (out-of-core) queries", description="Allow generated code to call execute_query_lazy, which streams query results to a Parquet spill file on disk and returns an out-of-core LazyFrame instead of an in-memory DataFrame. Off by default while the streaming path matures.", is_lab=True, editable=True)
