...
class OrganizationSettingsConfig(BaseModel):
    ...
    enable_mcp_native_tools: FeatureConfig = FeatureConfig(value=True, name="Native MCP tool registration", description="Give the agent one directly callable tool per external MCP / custom API tool, carrying the server's own argument schema, instead of routing every call through the generic execute_mcp tool. Only takes effect once a report's tool catalog reaches the threshold below. Policies, per-user identity forwarding and audit are unchanged either way.", is_lab=True, editable=True)
