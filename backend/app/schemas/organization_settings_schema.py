...
class OrganizationSettingsConfig(BaseModel):
    ...
    load_step_max_age_seconds: FeatureConfig = FeatureConfig(value=300, name="Reusable step recency window (seconds)", description="Only advertise steps built within this window as reusable via load_step. Set to 0 for no limit. Re-running saved code is unaffected.", is_lab=False, editable=True)
