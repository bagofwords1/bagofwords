...
class OrganizationSettingsConfig(BaseModel):
    ...
    enable_lazy_queries: FeatureConfig = FeatureConfig(value=False, name="Lazy (out-of-core) queries", description="Allow generated code to call execute_query_lazy, which streams query results to a Parquet spill file on disk and returns an out-of-core LazyFrame instead of an in-memory DataFrame. Note: generated code is not yet instructed to use this API — enabling only unlocks the capability. Lazy results coerce nested values to JSON strings and high-precision decimals to floats (see DataSourceClient.execute_query_lazy). Off by default while the streaming path matures.", is_lab=True, editable=True)
