...
class OrganizationSettingsConfig(BaseModel):
    ...
    query_timeout_seconds: FeatureConfig = FeatureConfig(value=60, name="Query timeout (seconds)", description="Default per-query wall-clock timeout when the agent runs SQL via create_data / inspect_data. A connection's config can override this with its own 'query_timeout_seconds' value.", is_lab=False, editable=True)
