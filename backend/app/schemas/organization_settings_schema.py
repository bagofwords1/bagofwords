...
class OrganizationSettingsConfig(BaseModel):
    ...
    enable_artifact_store: FeatureConfig = FeatureConfig(value=True, name="Investigation Artifact Store", description="Persist full results of large queries as encrypted, sliceable artifacts (DuckDB) so the agent can page/grep/aggregate beyond the row cap. Results below the cap are unaffected.", is_lab=True, editable=True)
