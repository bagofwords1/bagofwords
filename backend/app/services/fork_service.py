    ...
    async def _duplicate_assets(
        ...
        for old_query in original.queries:
            ...
            if old_step is not None:
                ...
                # On a strict source these are the filter values the forker's
                # first run starts from (hydration and refresh-on-view both
                # rerun from Step.applied_params) — exactly what the read path
                # serves this forker as a reader, nothing at all when their
                # read is withheld, so the fork never hands over more than the
                # share already did.
                read_withheld = strict_source and (
                    await resolve_step_data(db, old_step, original, user)
                ).withheld
                new_step = Step(
                    ...
                    # applied_params records the values the snapshot was
                    # materialized with. Carried alongside a copied snapshot,
                    # because the dashboard reads it to tell a pre-filtered
                    # snapshot from an unfiltered one — without it,
                    # ArtifactFrame derives filter options from rows the
                    # creator had already narrowed and offers a one-value list.
                    # Carried WITHOUT the snapshot on a strict source, as the
                    # values the first run executes with: dropped, the fork
                    # opened on the declared defaults instead of the year /
                    # month / agent the dashboard was showing. Identity-sourced
                    # values are stripped either way (the same boundary
                    # redact_applied_params draws for a reader): those name the
                    # creator, not the data, and the rerun re-derives them from
                    # the forker.
                    applied_params=redact_applied_params(
                        old_step.applied_params, old_query.parameters,
                        withheld=read_withheld,
                    ),
