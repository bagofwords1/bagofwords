...
def test_completion_creation(
    ...
    login_user,
    create_organization,
    create_azure_provider_and_models,
    get_default_model
):
    ...
    # Skip if Azure test variables are not set
    if not os.getenv("AZURE_API_KEY_TEST"):
        pytest.skip("AZURE_API_KEY_TEST is not set")
    if not os.getenv("AZURE_ENDPOINT_TEST"):
        pytest.skip("AZURE_ENDPOINT_TEST is not set")

    provider_id = create_azure_provider_and_models(user_token, org_id)
