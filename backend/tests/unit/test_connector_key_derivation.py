...
renders the brand icon (Gmail, Notion, …) instead of the generic MCP glyph.
...
only its `server_url` (e.g. Gmail) came back with `connector_key=None` in report
responses, and the tool-call rows showed the generic stack/MCP icon.
"""
from app.schemas.data_source_schema import ConnectionEmbedded, _connector_key_from_config


GMAIL_SERVER_URL = "https://gmailmcp.googleapis.com/mcp/v1"
...
def test_connector_key_from_server_url_preset_match():
    ...
    assert _connector_key_from_config({"server_url": GMAIL_SERVER_URL}) == "gmail"


def test_connector_key_accepts_json_string_config():
    assert _connector_key_from_config('{"server_url": "%s"}' % GMAIL_SERVER_URL) == "gmail"
...
def test_connection_embedded_resolves_gmail_from_server_url():
    ce = ConnectionEmbedded(
        id="c1", name="Gmail", type="mcp",
        config={"server_url": GMAIL_SERVER_URL},
    )
    assert ce.connector_key == "gmail"


def test_connection_embedded_explicit_key_wins():
    ce = ConnectionEmbedded(
        id="c2", name="My Notion", type="mcp",
        config={"catalog_key": "notion", "server_url": GMAIL_SERVER_URL},
    )
