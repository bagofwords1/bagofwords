...
def test_prompt_is_parallel_only(monkeypatch):
    """Parallel emission is the ONLY prompt protocol — batching language is
    present regardless of the org flag or env overrides (the flag still
    governs the API-level and execution-side behavior, not the prompt)."""
    ...
    for flag in (False, True):
        system = PromptBuilderV3._build_system(
            PlannerInput(user_message="x", parallel_tools_enabled=flag)
        )
        assert "BATCH independent calls" in system
        assert "HARD RULE" not in system
        assert "MULTI-TOOL" not in system
        assert "at most one tool call per turn" not in system
...
async def test_code_executions_overlap_without_global_lock():
    ...
    # Warm-up: generated code runs in pre-spawned sandbox children; the very
    # first execution in a process pays the interpreter start. This test
    # measures overlap, not cold start, so fill the warm pool first.
