...
- **Trino data source** — new connector for the Trino distributed SQL engine (the Presto successor) via the official `trino` DB-API driver. A connection binds to a single **catalog**; tables are discovered from that catalog's `information_schema.columns` and surfaced as `schema.table`, with an optional comma-separated **Schema** filter (all non-system schemas indexed when blank). Username/password auth is supported and automatically upgrades to HTTPS basic auth when a password is set. Default coordinator port `8080`. Ships with a Trino icon in the data-source picker.
