...
## Unreleased
- Added a **Graph Permission Model** choice to the SharePoint connector, for tenants whose admins will not consent to a site-wide read. "Selected sites only" asks user sign-in for `Sites.Selected` instead of `Sites.Read.All` + `Files.Read.All`, so the app registration can reach only the sites an admin has explicitly granted it — narrowing enforced by Microsoft rather than by the connection's folder and pattern settings. Existing connections keep the scopes their users already consented to; switching an existing connection over asks its users to sign in again. The per-site grant that `Sites.Selected` requires (`POST /v1.0/sites/{site-id}/permissions`) is a separate one-time step, and the connection test now says so instead of passing Graph's bare "Doesn't have the required Permission scopes to access a site" through. A SharePoint 403 also now distinguishes its two causes, which need opposite fixes and were previously indistinguishable: a token missing a site permission (an admin grants it) versus a signed-in user who simply cannot open that site (their SharePoint access, or the wrong site URL)
