...
## Version 0.0.426 (June 26, 2026)
- **QVD — date columns are now real dates you can `CAST` and compare** — a Qlik date/timestamp field whose QVD header carried no `$date`/`$timestamp` tag and an `UNKNOWN` NumberFormat `Type` used to be converted to a raw Excel-style **serial number** (e.g. `46174`), so `WHERE CAST(s.Date AS DATE) = '2026-06-01'` failed (`Unimplemented type for cast … -> DATE`) and only clumsy numeric workarounds ran. The `qvd2parquet` converter now also recognizes such fields by their **display-format pattern** (`YYYY-MM-DD` → `DATE`, `DD/MM/YYYY hh:mm:ss` → `TIMESTAMP`, `hh:mm:ss` → `TIME`) and by the numeric Type codes some writers emit (`1`=date, `3`=timestamp), emitting them as proper `DATE`/`TIMESTAMP`/`TIME`. Numeric/currency formats (`#,##0.00`, `###0`) are explicitly left alone (any digit in the pattern means "not a date"), and a date-named **text** column still stays text. The Parquet cache schema version is bumped (`v3`), so existing caches re-convert automatically on the next warmup, and the pre-warmup schema hint mirrors the same detection.
