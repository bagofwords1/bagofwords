...
## Version 0.0.483 (July 25, 2026)
- **Google Chat joins Slack, Teams, and WhatsApp as a chat channel** — users DM the org's Chat app (or @-mention it in a space) and get the analyst's answers threaded back, with Workspace-email auto-linking, per-agent channel availability, the `"new"` fresh-conversation command, and an org-configurable DM session window (default 5 days). Delivery uses Google Chat's **Cloud Pub/Sub connection mode**: the app publishes events to a topic in the customer's GCP project and the backend pulls them outbound, so — unlike every other chat integration — **no public URL, webhook, or inbound firewall access is required**, making it the first chat channel deployable in fully locked-down environments. Since Chat apps can't add reactions, an editable "👀 Working on it…" marker message threads under the user's question while the agent runs and is removed when the answer lands. The setup modal walks through the GCP steps (dedicated project, service account, topic + subscription with the two IAM grants) and the connect flow validates the whole chain — Chat API token + a real subscription pull — before saving. Proactive nudges (report shares, agent notifications) also route to Google Chat for users with an existing DM with the app, falling through to email otherwise. The full loop was verified end-to-end against a live Workspace tenant, and the design/troubleshooting notes (including the Workspace add-on checkbox trap and Domain Restricted Sharing workaround) live in `docs/design/google-chat-integration.md`.
