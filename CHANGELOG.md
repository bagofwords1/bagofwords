...
- **Added a Priority ERP connector (beta)** — Priority Software's ERP, cloud and on-premise, over its OData v4 REST API. Each Priority **form** is indexed as a table with its subforms as join paths, and — the detail that makes the catalog usable — column **titles** are read from Priority's `Priority.OData.Description` annotations rather than the bare `<Property>` elements, so the agent sees "Customer Number" alongside `CUSTNAME` instead of a wall of cryptic names. Titles follow the tenant's environment language (a Hebrew tenant stays Hebrew), so names remain the identifiers throughout. Three auth modes: a Personal Access Token (sent as Priority documents it — token as the Basic username with the literal password `PAT`), a dedicated API user, and per-user OAuth sign-in — the last **on-premise only**, since Priority scopes its OAuth2 support to non-SaaS installations and requires the External ID module; Priority Cloud gets per-user access via bring-your-own token instead. OAuth endpoints are derived per tenant from the OData service root (preserving IIS virtual directories), a service credential keeps driving catalog indexing while queries run under the signed-in user, and Cloud's 100 calls/minute limit is respected but configurable so on-premise installs aren't throttled for no reason. Priority documents no `$apply`, so aggregation requests fail with the limitation and the workaround named rather than a bare 501. Verified end-to-end against a protocol-level mock (`tools/priority/mock_server.py`) that reproduces Priority's annotation-based metadata, PAT auth shape, subform expansion, 429 throttling and missing `$apply`.
