...
- **Projects: private, shareable folders for reports and dashboards** (#803) — group related reports and dashboards into a project, invite specific teammates who see everything in it read-only and can fork any report to continue the analysis themselves, and set the workstream up once: default agents are attached to every new report created in the project, project files are inherited live into the AI's context (added later? existing reports see them too), and project instructions ("always report revenue in EUR") steer every answer. The project page shows reports, dashboards, automations (scheduled tasks and dashboard refreshes), members, agents, files and instructions on one screen; the prompt box gets a project chip for moving a report in or out. Deleting a project archives its reports and stops their scheduled tasks (the confirmation spells out exactly what gets archived, stopped, and unlinked), while a scheduled dashboard refresh keeps running as long as the dashboard is still published. A /projects page lists every project you own or were invited to, the sidebar shows project reports with color-tinted icons, and sharing is safe by construction: a member can only be invited if they can access every project agent, and only agents every member can access may become project defaults — enforced server-side either way
