...
- The **Agents** page no longer waits on pending-change reconciliation. Showing which instructions have a change awaiting review meant re-diffing every unreviewed suggestion against the current text on every page load — work that grew with how far the live instructions had moved on since those suggestions were written, and that ran on long text. A workspace with 139 instructions and 169 unreviewed suggestions waited 23 seconds for the instruction counts and 20 for an agent's instruction list; both now answer in well under a second. The review itself is unchanged: opening an instruction still computes its changes exactly, and a dot that turns out to have nothing left to review clears itself
- Fixed the **All instructions** count reporting more instructions than exist — a proposal that was never published was counted both as pending and as not-live. A workspace with 139 instructions showed 220
- Very long instructions (past roughly 8,000 characters) are now reviewed as a single whole-text change rather than phrase by phrase. Comparing them word by word cost seconds per suggestion, which is what made a page full of them unusable
