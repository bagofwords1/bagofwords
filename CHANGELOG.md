...
- **Results from an MCP or custom-API tool now reliably reach the tools that analyze them.** A tool returning a few hundred work orders could end a turn with no table at all: the agent fetched the data, then `write_csv` and `create_data` failed repeatedly, and the run finished by summarizing a truncated preview as if it were the full result. Several things had to line up for that, and all of them are fixed. Every successful call now saves its result to a file **exactly as the server sent it** and hands the next tool that file's id — previously a result the detector didn't recognize as a table was dropped, and a failed save was swallowed into a log line that looked identical to "there was nothing to save". Detection got looser (records under a domain-specific key, with a warnings list beside them, no longer defeat it) but is now advisory: it reports *where* the records are rather than deciding what survives to disk, so a wrong guess costs nothing and the payload's pagination cursors, sibling lists and nested fields are all preserved. `create_data`, `write_csv` and `inspect_data` accept the file id directly and are told the exact path, reader and columns, so the generated code opens the file it was given instead of inferring which attachment was meant. MCP connections are no longer offered to generated code as if they were queryable databases — the failing runs ended with the model writing `ds_clients["Agent:Conn"].execute_query(...)`, a call that exists on no client, because the data it needed wasn't anywhere it could reach
- **Text and document results are handled honestly.** A plain-text result (a log, handover notes, a transcript) used to be truncated to a preview with the remainder discarded; it is now saved in full, and generated code can read it — previously the only available call was `pd.read_csv`, which on prose does not fail but returns a plausible-looking table built from wherever the commas happened to fall. Formats that generated code genuinely cannot open (PDF, Word, PowerPoint, images) are now labelled as such, so the agent routes them to the file reader instead of attempting a load that can only fail
- **Very large tool responses no longer risk taking down the worker.** A response beyond a size threshold is written to disk in full but not parsed in memory, where it would have cost several times its size; the agent is told the file is complete but its structure was not inspected, so it reads the file rather than assuming the result was empty
- Fixed a summary table silently showing raw rows instead of the summary: a generated function ending in `return df_aggregated` was rewritten to `return df`, shipping the pre-aggregation frame with nothing reporting an error
- Fixed a file saved during a turn not being readable until the next turn
