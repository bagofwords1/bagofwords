...
- **Fix — `create_data` preview truncated to 5 rows in planner context (#471)** — in prompt builder v3 the planner could reason over only 5 rows (or none, for older steps) even when more were generated. The hard-coded `rows[:5]` slice and observation-compaction that deleted `data_preview` entirely are replaced by a single budgeted preview helper (`build_data_preview`): the latest observation carries as many rows as fit a ~48 KB byte budget (small results come through whole; large ones degrade to a labeled head + tail with the true `row_count`), and compaction keeps a labeled 3-row sample instead of dropping the preview. Planner guidance now says to trust `row_count` over a partial preview.
