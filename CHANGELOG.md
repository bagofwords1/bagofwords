...
- **Fix — false "non-admin proposed" notifications when an agent admin adds an instruction** — the per-user draft build was reused org-wide (`get_or_create_draft_build` for `source='user'` ignored the creator), so an unpromoted draft left behind by agent generation (the onboarding instruction, created with `auto_finalize=False`) — or another user's pending change — got merged into the next user's instruction create. The finalize check then evaluated the whole shared build and flagged the user's own, properly-scoped change as un-publishable, firing a spurious "A non-admin proposed this instruction change — review and accept or run an eval." notification to admins even though the user had published to their own agent. The same entanglement could also promote one user's pending instruction to main on another user's publish. Draft-build reuse is now scoped to its creator (`created_by_user_id`), so each user accumulates and finalizes only their own pending changes.
