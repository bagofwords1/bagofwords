...
- **Power BI queries now aggregate and filter inside DAX instead of pulling the table into pandas.** For "total quantity by product in a date range" the agent ran an unfiltered `SUMMARIZECOLUMNS`, then `EVALUATE FILTER(Table, <dates>)` over every row and column, and grouped in pandas — the raw transfer through `executeQueries` was the slow part (a customer measured ~90s vs ~15s for the same question with a hand-written DAX). The DAX guide gains a Query Efficiency section (filter as a `SUMMARIZECOLUMNS` argument or in `CALCULATETABLE`, `SELECTCOLUMNS` projections for row listings, `&&`/`||` rather than the infix `AND`/`OR` that also failed live), the coder's Power BI rule says to push both into the query, and the legacy planner (`BOW_PLANNER=v2`) no longer tells the model to return granular rows for "by X" questions. Verified live against a Power BI tenant: one filtered query, 8 rows back, same totals; row-listing asks still return rows. See `docs/feedback-loops/powerbi-dax-efficiency.md`.
