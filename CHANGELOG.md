...
- **Added Google Chat as a chat channel** — DM the org's Chat app or @-mention it in a space and get answers threaded back, with Workspace-email auto-linking and proactive notifications. Uses Google Chat's Pub/Sub connection mode (events are pulled outbound), so no public URL or inbound firewall access is needed. Setup guide in the integration modal; design notes in `docs/design/google-chat-integration.md`.
