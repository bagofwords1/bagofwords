...
## 6f. NetApp ONTAP — the build plan (verified against the spec)

### What was read

- NetApp's **public ONTAP REST OpenAPI (Swagger 2.0) spec**, the inline
  JSON embedded in `NetAppDocs/ontap-restapi` `swagger-ui/index.html`
  (587 paths; the same document a cluster serves at `/docs/api/`). Every
  endpoint, parameter and field below is taken from it. Security scheme in
  the spec: HTTP basic only (`simple: basic`); OAuth2 bearer is an ONTAP
  9.14+ authentication *setting*, not a spec change, so the client sends
  either a basic pair or a bearer token on the same calls.
- The NetApp docs read earlier (§4.1): OAuth2 (9.14+), the 20-session cap
  per User-Agent, ZAPI deferral, the read-only role recipe, AIQUM `/api/v2`.

Confirmed in the spec:
- Inventory endpoints exist with GET: `/cluster`, `/cluster/nodes`,
  `/svm/svms`, `/storage/aggregates`, `/storage/volumes`, `/storage/luns`,
  `/storage/qtrees`, `/storage/disks`, `/storage/shelves`,
  `/network/ip/interfaces`, `/network/fc/ports`, `/snapmirror/relationships`,
  `/storage/qos/policies`, `/protocols/san/igroups`, `/protocols/san/lun-maps`.
- **History is per object only.** `/metrics` exists on volumes, aggregates,
  LUNs, nodes, cluster, IP/FC interfaces, qtrees, NVMe namespaces, protocol
  services; there is no collection-level `/storage/volumes/metrics`. Query
  param `interval` ∈ `1h|1d|1w|1m|1y` (15 s / 5 min / 30 min / 2 h / 1 day
  samples). Records: `timestamp` (ISO-8601), `duration` (`PT15S`…`P1D`),
  `status` (`ok`, `partial_no_data`, `backfilled_data`, …), `iops.{read,write,
  other,total}`, `latency.{…}` (µs), `throughput.{…}` (B/s).
- **"Current" is fleet-wide in one call:** every volume/aggregate/LUN/node
  record carries an embedded `metric` (latest averaged sample, same shape)
  and `statistics` (raw monotonic counters). `GET /storage/volumes?fields=
  name,svm.name,state,space.size,space.used,metric` returns the whole
  estate's latest IOPS/latency in one paged call.
- Events: `GET /support/ems/events` → `index`, `time`, `node.name`,
  `message.name`, `message.severity` (`emergency|alert|error|notice|
  informational|debug`), `log_message`, `parameters[]`, `source`;
  filterable on every field, plus `filter.name` for a saved EMS filter.
- Raw counters: `/cluster/counter/tables` (`name`, `description`,
  `counter_schemas[]`) and `/cluster/counter/tables/{name}/rows`
  (`id`, `properties[]`, `counters[]{name, value|values, labels}`,
  `aggregation`).
- Common query params on every collection: `fields`, `max_records`,
  `return_records`, `return_timeout`, `order_by`, and per-field filters
  (`state=online`, `space.used=>1000000`, `name=vol_prod*`).
- Join keys: `lun.serial_number` ("generated by ONTAP when the LUN is
  created") and `serial_number_hex`; `volume.uuid`, `svm.uuid`,
  `node.serial_number`, `cluster.uuid`; `volume.state`
  (`online|offline|restricted|mixed|error`), `volume.type` (`rw|dp|ls`),
  `volume.style` (`flexvol|flexgroup|flexgroup_constituent`).

### Endpoint → table map

| Table | Call | Rows |
|---|---|---|
| `cluster` | `GET /cluster?fields=name,uuid,version,contact,location,metric` | one row |
| `nodes` | `GET /cluster/nodes?fields=name,uuid,model,serial_number,state,uptime,version,ha,metric` | node |
| `svms` | `GET /svm/svms?fields=name,uuid,state,subtype,nfs,iscsi,fcp,nvme,aggregates` | SVM |
| `aggregates` | `GET /storage/aggregates?fields=name,uuid,node,state,space.block_storage.{size,used,available},space.efficiency.ratio,metric` | aggregate |
| `volumes` | `GET /storage/volumes?fields=name,uuid,svm,aggregates,state,type,style,size,space.{size,used,available},space.logical_space.used,snapshot_count,qos,metric` | volume |
| `luns` | `GET /storage/luns?fields=name,uuid,serial_number,serial_number_hex,svm,location,os_type,space.{size,used},status,enabled,lun_maps,metric` | LUN |
| `qtrees` | `GET /storage/qtrees?fields=name,id,volume,svm,path,export_policy` | qtree |
| `ip_interfaces` / `fc_ports` | `GET /network/ip/interfaces` / `GET /network/fc/ports` | interface / port |
| `snapmirror` | `GET /snapmirror/relationships?fields=source,destination,state,healthy,lag_time,last_transfer_error` | relationship |
| `ems_events` | `GET /support/ems/events?time=>{iso}&message.severity=alert,error` | event |
| `metrics` | `GET /storage/{volumes|aggregates|luns}/{uuid}/metrics?interval=…`, `/cluster/nodes/{uuid}/metrics`, `/cluster/metrics` | (object, timestamp) row with iops/latency/throughput split read/write/other/total, `status`, `duration` |
| `metrics_current` | the embedded `metric` on the inventory endpoints | one row per object, fleet-wide, one call |
| `counter_tables` / `counter_rows` | `/cluster/counter/tables`, `…/{name}/rows` | dictionary / raw counters |

Discovered tables are **not** needed here: ONTAP's object model is fixed,
so the catalog is declared in code (Zabbix shape), not discovered (Aria
shape). Descriptions get enriched at index time with cluster name/version,
SVM names, aggregate and volume counts.

### `execute_query` spec

```python
{"table": "volumes", "filter": {"svm.name": "svm_prod", "state": "online", "name": "vol_prod*"}, "fields": [...], "limit": 500}
{"table": "metrics", "object": "volumes", "name": "vol_prod_db", "interval": "1d"}                 # or "uuid": "…"
{"table": "metrics", "object": "aggregates", "uuid": "…", "interval": "1w", "start_time": "2026-09-03T01:00:00Z", "end_time": "…"}
{"table": "metrics_current", "object": "volumes", "filter": {"svm.name": "svm_prod"}}
{"table": "ems_events", "severity": ["alert", "error"], "start_time": "2026-09-03T02:00:00Z", "end_time": "…", "node": "node-01"}
{"table": "counter_rows", "counter_table": "volume", "filter": {"properties.name": "…"}}
```
Rules: filters map 1:1 to ONTAP query params (operators `*`, `<`, `>`,
`..`, `!`, `|` pass through); `fields` default to the catalog's curated
list (never `fields=*`); pagination follows `_links.next.href`; `metrics`
fans out one call per object, so `name`/`filter` scope is required and
capped (default 50 objects); `interval` is chosen from the window
(≤1 h → `1h`, ≤1 day → `1d`, ≤1 week → `1w`, ≤1 month → `1m`, else `1y`)
and rows outside `start_time`/`end_time` are trimmed client-side, since
the API has no begin/end.

### Config, auth, gates

- `NetAppOntapConfig`: `url` (cluster management LIF), `verify_ssl`,
  `ca_bundle`, `default_interval` (`1d`), `metrics_fanout_limit` (50),
  `svm` (optional: scope every query to one SVM, for per-tenant
  connections).
- Credentials: `userpass` (basic; a `security login rest-role` read-only
  user) and `bearer` (OAuth2 JWT from Keycloak/Entra on 9.14+); both
  `scopes=["system","user"]`. Client cert is a follow-up (needs a
  cert/key upload path in the form).
- A distinctive `User-Agent: bagofwords-netapp/<version>` and `return_timeout=60`
  on every call (the 20-session cap is per User-Agent; the default 15 s
  timeout truncates big collections).
- Registry: `category="infra"`, `version="beta"`, `requires_license="enterprise"`,
  explicit `client_path`, plus `ENTERPRISE_DATASOURCES`. Icon: NetApp logo.
- No new dependency (`requests`).

### Test target

- **Mock** `tools/netapp_ontap/mock_ontap_api.py` generated from this spec:
  the 15 catalog endpoints, HAL `_links.next` paging, `fields`/filter
  operators, per-object `/metrics` with the five `interval`s and correct
  `duration`, embedded `metric` on inventory, EMS events, counter tables.
  Seeded estate: 2-node cluster, 2 SVMs, 3 aggregates, ~15 volumes, 6 LUNs
  (serials matching the Aria mock's datastore NAA ids so cross-connector
  joins work), and the **same incident** (a batch volume on `aggr1_ssd`
  saturates it; `vol_prod_db` latency follows; EMS `wafl.*`/`qos.*`
  events in the window). Faithfulness rules: basic/bearer required (401),
  `max_records` > 10 000 → 400, `return_timeout` honoured, `fields=*`
  answered but slow-flagged, 20th concurrent session → 429-like error.
- **Contract check in CI**: `stoplight/prism` container serving the same
  spec, hit by the unit suite's fixtures to catch shape drift.
- **Live leg**: NetApp Simulate ONTAP (customer/partner support login,
  VMware Workstation or KVM) — the client is the same; run
  `tests/integrations/ds_clients.py -k netapp_ontap` against it. Ask the
  customer for that login or for a read-only user on a non-production
  cluster.

### Phases

0. Customer inputs: ONTAP version(s), whether AIQUM/Harvest exist, a
   read-only REST role user, the cluster management URL and CA.
1. Mock from the spec (+ seeded incident aligned with the Aria mock).
2. Client + schemas + registry + icon; `system_prompt` with the RCA
   examples and the join keys (LUN serial ↔ datastore NAA id, volume
   name/SVM ↔ ServiceNow storage volume CI).
3. Unit tests at the `requests` boundary (auth variants, paging,
   fields/filters, interval selection + trimming, fan-out cap, EMS
   filters); integration entry; e2e suites.
4. Sandbox loop: connect wizard, index (~15 tables), three prompts —
   "which LUN backs datastore ds_prod_db_01 and what is its latency
   today", "was aggr1_ssd saturated at 02:10 and by which volume", "EMS
   alerts in the last 24 h by node" — screenshots + loop doc.
5. Optional AIQUM mode later (fleet inventory + `/api/v2` metrics with
   `interval` up to 6 months) if the customer runs it.

Size: comparable to Aria (client ~700 lines, mock ~600). Two PRs again.
...
### NetApp
- https://github.com/NetAppDocs/ontap-restapi/blob/main/swagger-ui/index.html (inline OpenAPI spec, read directly for §6f)
