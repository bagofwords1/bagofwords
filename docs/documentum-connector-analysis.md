...
| `elastic/connectors` — `connectors/sources/opentext_documentum.py` (branch 8.14; **removed from `main`**) | Python 3, `aiohttp` | REST (`/dctm-rest`) | Basic; header `content-type: application/vnd.emc.documentum+json` | Apache-2.0 / Elastic; "example connector, as-is"; no permission sync; full sync only; 10 MB extraction cap | **Closest thing to our connector in Python** — 499 lines; URL templates for repositories/cabinets/folders/recursive folders/documents with `page`/`items-per-page`, walks `entries[]`, uses `id`, `title`, `updated`, `size`; handles 429 `Retry-After`. Good fixture/shape reference. |
