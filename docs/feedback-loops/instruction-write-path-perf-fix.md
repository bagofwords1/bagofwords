...
## A claimed bug that was not one

The earlier doc reported `_can_auto_publish_build` as buggy: it loads the build's
whole snapshot, so a single global instruction anywhere in the org blocks an
agent admin from auto-publishing. Filtering it to `is_change` broke six tests in
`tests/e2e/rbac/test_instruction_pending_carryover.py`, whose fixture says so
outright:
...
It is a deliberate authorization boundary, and it guards something real:
promoting a build replaces main with its **whole snapshot**, so a stale one can
revert instructions changed since it forked. Requiring authority over everything
the build *contains* is what stops an agent admin from promoting a build that
reverts another agent's work. The change was reverted and the reasoning recorded
in the code. That cost goes away only when drafts stop carrying full snapshots —
