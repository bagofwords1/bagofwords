...
function App() {
  ...
  const selectAlbum = id => {
    ...
    if (window.matchMedia('(max-width: 900px)').matches) {
      requestAnimationFrame(() => document.querySelector('.detail-panel')?.scrollIntoView({ block: 'start' }));
    }
  };
  ...
  return (
    <div className="listening-room">
      <style>{`
        ...
        .lr-return { display: none; }
        @media (max-width: 900px) { .lr-return { display: inline-flex; margin-bottom: 18px; } }
        .lr-kicker {
          color: var(--rust);
          font-size: 11px;
          letter-spacing: .18em;
      ...
      <div className="lr-shell">
        <header>
          <div className="lr-kicker">Music collection</div>
          <h1 className="lr-title">Listening Room</h1>
        ...
        <main className="lr-layout">
                ...
                {sortedAlbums.map(row => {
                  ...
                  return (
                    <article
                      ...
                      onClick={() => selectAlbum(row.album_id)}
                      onKeyDown={event => {
                        if (event.key === 'Enter' || event.key === ' ') {
                          event.preventDefault();
                          selectAlbum(row.album_id);
                        }
                      }}
                      tabIndex={0}
          ...
          <aside className="detail-panel" aria-labelledby="detail-heading">
            <button type="button" className="lr-button secondary lr-return" onClick={() => document.querySelector('.album-row.selected')?.scrollIntoView({ block: 'center' })}>Back to albums</button>
            {!selectedAlbum ? (
              <div className="empty">
              ...
              <div data-bow-viz="FIXTURE_VIZ_0">
                ...
                <p className="lr-note">
                  Track details reflect the current catalog filters.
                </p>
              </div>
            )}
          </aside>
